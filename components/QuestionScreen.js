...
/*
  Question Screen: Displays a question for the selected location
*/

export default function QuestionScreen({ route, navigation }) {
  const [check, setCheck] = useState(1); //used to check only 1 answer (like radio button)
  let [shuffleChoices, setChoices] = useState([]); //to shuffle the choice for the selected location (...needs work)
  const {title, description, parsed, places_id} = route.params //parameter passed down from sites screen

  //loop over each location description and make checkboxes
  let choices = parsed.map((place, index) => {
    ...
    //search the title in the description, and replace it with an empty string
